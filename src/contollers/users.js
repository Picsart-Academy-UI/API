...
const {excludeUndefinedFields} = require('../utils/util');
...
exports.getUsers = asyncHandler(async (req, res, next) => {
  ...
  if (!team_id) {
    return next(new ErrorResponse('team_id not found', 400));
  }
  const users = await UserModel.find({team_id})
    .exec();
...
exports.getMe = asyncHandler(async (req, res, next) => {
  const {_id} = req.user;
  const found_user = await UserModel.findById(_id)
    .exec();
  if (found_user) {
    return res.status(200)
      .json({
